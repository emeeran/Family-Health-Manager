...
export function useRecordFormState({
  ...
  // After a successful *create*, clear the form so the next record can be entered
  // straight away, and optionally offer to sync the active-medication list. Record
  // *updates* never reach here — the edit action navigates itself and returns null.
    ...
    if (prevPendingRef.current && !isPending && state) {
      ...
      if (result.success) {
        ...
        toast.success("Record created", {
          description: "Form cleared — add another, or view this record.",
          action: recordId
            ? { label: "View", onClick: () => navigate(`/people/${memberId}/records/${recordId}`) }
            : undefined,
        });
        // Reset on every success path. The medication-sync dialog renders from
        // `medSyncDiff` (independent of form state), so clearing the form first is
        // safe even when the dialog is about to open.
        resetForm();
        if (result.prescriptions && memberId) {
          ...
          import("@/lib/api/members").then(({ computeMedicationDiff }) => {
            computeMedicationDiff(memberId, rx, recordId as string)
              ...
              .catch(() => {
                /* med-sync is best-effort; the form is already reset */
              });
          });
        }
