...
class MedicationService:
    ...
    async def get_active_medications(self, member_id: UUID) -> list[dict]:
        ...
        Reads from record JSON (``health_records.clinical_data``), the system of
        record — every writer (the records router via ``sync_from_record`` *and*
        the dedicated medication CRUD endpoints) edits JSON. The ``medications``
        table is a write-only cache and is intentionally not consulted here:
        consulting a partial/stale cache is what hid medicines added or edited
        from the "Currently Taking" card for members who already had table rows.
        """
    ...
    async def get_refill_reminders(self, member_id: UUID) -> list[dict]:
        ...
        Derived from the JSON-authoritative ``get_active_medications`` (see why
        there). Filters to active meds whose end_date falls within the next 7
        ...
        reminders: list[dict] = []
        for med in await self.get_active_medications(member_id):
            if med["status"] != "active" or not med["end_date"]:
                continue
