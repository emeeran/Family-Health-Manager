...
_COMPREHENSIVE_INSIGHT_PROMPT = (
    "You are writing a formal clinical health assessment report — the kind a senior physician "
    "would produce after a thorough chart review. You MUST reference every condition, diagnosis, "
    "provider, and medication found in the patient data. Do NOT omit any named conditions, "
    "diagnoses, doctors, or providers mentioned in the records.\n\n"
    ...
    "Use exactly these 7 sections:\n\n"
    "1. **Health Overview** — Open with the patient's age, sex, and primary clinical concern. "
    ...
    "2. **Active Conditions** — For EACH diagnosed condition, assess its current trajectory "
    ...
    "3. **Medication Analysis** — List each active medication with its dosage and frequency. "
    "State the indicated condition ONLY if explicitly documented in the records — otherwise write "
    "'indication per records: not specified.' "
    "Evaluate: (a) dosage appropriateness relative to the condition, "
    "(b) potential drug-drug interactions between concurrent medications, "
    "(c) potential drug-condition contraindications, "
    "(d) duplicate therapy across providers. "
    "Flag any medication without a clear indication in the records.\n\n"
    "4. **Lab Trends** — For each lab test with serial results, present: date, value, reference range, "
    ...
    "5. **Risk Assessment** — Identify specific risks derived from the COMBINATION of this patient's "
    ...
    "6. **Recommendations** — Every recommendation must follow: specific finding → clinical implication → "
    ...
    "7. **Follow-up Actions** — List by clinical priority (urgent > important > routine). "
...
_PRE_CONSULT_PROMPT = (
    "Generate a pre-consultation clinical inquiry for the patient. "
    "Use patient data: profile, history, visits, labs, prescriptions, vitals."
    "{specialty_section}"
    "{symptoms_section}\n\n"
    "FORMAT — output EXACTLY this structure:\n\n"
    "### **Pre-Consultation Clinical Inquiry**\n"
    "*(The following points are generated based on the patient's symptoms and medical history "
    "for discussion with the attending physician.)*\n\n"
    "Then list 6-10 items. Each item MUST be a single bullet in this exact pattern:\n"
    "* **Category (Detail):** Specific clinical question referencing patient data.\n\n"
    "CATEGORIES (use the most relevant ones):\n"
    "- **Medication Interaction**: Ask about concurrent use, contraindications, or interactions between specific medications the patient takes.\n"
    "- **Symptom Assessment (Area)**: Ask about the etiology, significance, or workup for a reported symptom. Always name the body area.\n"
    "- **Lab Result Interpretation**: Ask what a specific abnormal or borderline lab value implies given the patient's conditions.\n"
    "- **Dosage Clarification**: Ask whether a current dosage is appropriate, needs adjustment, or should be changed.\n"
    "- **Follow-up Planning**: Ask when a repeat test, screening, or follow-up visit is due.\n"
    "- **Treatment Efficacy**: Ask whether a current treatment is working based on the latest data.\n"
    "- **Preventive Care**: Ask about overdue vaccinations, screenings, or lifestyle interventions.\n"
    "- **Referral Need**: Ask whether a specialist referral is warranted for a specific concern.\n"
    "{specialty_focus}\n"
    "RULES:\n"
    "- Every question MUST reference specific data from the patient's records (med names, lab values, dates, conditions).\n"
    "- Be clinically precise. Use medical terminology appropriate for physician discussion.\n"
    "- Prioritize actionable questions that will affect clinical decisions.\n"
    "- No preamble or closing text beyond the header and bullet list.\n"
    "- No filler or generic questions. Every item must be data-driven.\n"
)
