...
async def create_record(
    ...
    # (not spawn_insight_task/loop.create_task). get_db's commit runs AFTER background
    # tasks, so the explicit `await db.commit()` below is what makes the just-created
    # record visible to this task's own session (see _generate_insight_background).
    if background_tasks is None:
        background_tasks = BackgroundTasks()
...
async def update_record(
    ...
    # Sync medications and lab results SERIALLY on the request session. A
    # SQLAlchemy AsyncSession is not safe for concurrent use: the previous
    # asyncio.gather() over both syncs could trip a concurrent-use error that
    # each sync's own try/except silently swallowed — quietly dropping one or
    # both. Each is still independently wrapped so one failure doesn't skip the
    # other. (Mirrors create_record.)
    ...
    # Commit BEFORE returning. FastAPI runs BackgroundTasks during response send
    # (get_db is request-scoped, so its commit/close happens AFTER background
    # tasks). Without this explicit commit, the transcription-report task's own
    # session can't see the updated clinical_data and hits "database is locked"
    # on SQLite because the request session still holds the write lock. Mirrors
    # create_record.
    await db.commit()
